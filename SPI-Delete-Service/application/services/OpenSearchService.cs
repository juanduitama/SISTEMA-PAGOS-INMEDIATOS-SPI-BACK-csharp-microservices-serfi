    ...
    public class OpenSearchService : IOpenSearchService
    {
        ...
        public async Task <OSDefinitive> SearchKey(string keyType, string keyValue)
        {
            try
            {
                ...
                Console.WriteLine($"El objeto OSDefinitive es: " + await UtilCommons.Object2String(results));
        ...
        public async Task CancelledKey(string keyType, string keyValue, OSDefinitive osEntity)
        {
            try
            {
                ...
                osEntity.key.keyStatus = ValidationEnums.KEY_CANCEL_STATUS;

                // Eliminamos el documento usando su ID
                var deleteResponse = await _client.UpdateAsync<OSDefinitive,OSDefinitive>(documentId, d => d
                    .Index(_indexName)
                    .Doc(osEntity)
                );
